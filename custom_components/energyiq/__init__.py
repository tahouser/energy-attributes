...
from .shelly_ws import async_register as async_register_shelly_ws

PLATFORMS: list[str] = []
URL_BASE = "/energyiq-static"
FRONTEND_VERSION = "31700"
PANEL_PATH = "energyiq-3170"


async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    """Set up the EnergyIQ instrument services."""
    data = hass.data.setdefault(DOMAIN, {})
    if not data.get("_websocket_registered"):
        async_register_websocket(hass)
        async_register_shelly_ws(hass)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up one EnergyIQ instrument."""
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload one EnergyIQ instrument."""
