/* EnergyIQ v2.1.6 frontend loader. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=216`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub) sub.textContent = "Whole-home electrical intelligence · v2.1.6";
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched216) {
      ...
      proto.__energyiqVersionPatched216 = true;
    }
  }
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver216) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver216 = observer;
  }
