/* EnergyIQ v2.1.7 frontend loader. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=217`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub) sub.textContent = "Whole-home electrical intelligence · v2.1.7";
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched217) {
      ...
      proto.__energyiqVersionPatched217 = true;
    }
  }
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver217) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver217 = observer;
  }
