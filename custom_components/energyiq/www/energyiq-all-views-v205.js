/* EnergyIQ v2.0.5 load-list UX: All / Monitored / Excluded. */
(() => {
  ...
  const install = (panel) => {
    ...
    panel.addEventListener("click", (event) => {
      ...
      box.checked = next;
      if (next) pending.add(box.dataset.device); else pending.delete(box.dataset.device);
      panel._render();
    }, true);
    ...
    panel.addEventListener("change", (event) => {
      ...
      event.stopImmediatePropagation();
      const pending = ensurePending(panel);
      if (box.checked) pending.add(box.dataset.device); else pending.delete(box.dataset.device);
      panel._render();
    }, true);
    ...
    panel.addEventListener("click", async (event) => {
      ...
      } catch (e) { alert(`EnergyIQ could not perform that action: ${e.message || e}`); }
    }, true);
    ...
    panel._render = (...args) => {
      ...
      if (!bar) { bar = document.createElement("div"); bar.className = "energyiq-view-bar"; actions.appendChild(bar); }
      bar.innerHTML = `<span class="energyiq-view-label">Loads</span><button type="button" class="energyiq-view-btn" data-energyiq-view="all">All <span></span></button><button type="button" class="energyiq-view-btn" data-energyiq-view="monitored">Monitored <span></span></button><button type="button" class="energyiq-view-btn" data-energyiq-view="excluded">Excluded <span></span></button>`;
      ...
      for (const button of bar.querySelectorAll("[data-energyiq-view]")) {
        ...
        button.onclick = () => { panel._energyiqView = view; panel._render(); };
      }
      panel.querySelectorAll(".list-toggle").forEach(x => x.style.display = "none");

      const existing = new Map();
      table.querySelectorAll("input[data-device]").forEach(box => { const row = box.closest("tr"); if (row) existing.set(box.dataset.device, row); });
      ...
      for (const row of rows) {
        ...
        row.style.display = panel._energyiqView === "all" || (panel._energyiqView === "monitored" && monitored) || (panel._energyiqView === "excluded" && !monitored) ? "" : "none";
  ...
  const findPanel = () => { const panel = document.querySelector(TAG); if (panel) install(panel); };
  if (customElements.get(TAG)) findPanel(); else customElements.whenDefined(TAG).then(findPanel);
