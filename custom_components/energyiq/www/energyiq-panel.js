  ...
  class EnergyIQPanel extends HTMLElement {
    ...
    updateLiveData() {
      ...
      this.querySelectorAll("tr[data-device-id]").forEach(row => {
        ...
        const powerCell = row.querySelector(".power-cell"); if (powerCell) { powerCell.textContent = Number.isFinite(power) ? `${power.toFixed(0)} W` : "—"; powerCell.classList.toggle("capture-valid", fullCaptureValid); powerCell.title = fullCaptureValid ? `Capture valid · stable load ${Number(result.stable_load_w || liveDelta).toFixed(0)} W` : ""; }
        ...
        if (actionsCell) {
          ...
          const fullActive = training.status === "active" && training.method === "full_cycle"; const fullPhase = String(training.phase || ""); const action = fullActive ? (fullPhase === "awaiting_confirmation" || fullPhase === "waiting_for_start" ? `<button data-full-action="${this.attr(device.device_id)}" data-full-op="start_capture">Start Power Capture</button>` : fullPhase === "capturing" ? `<button data-full-action="${this.attr(device.device_id)}" data-full-op="stop_capture" ${result.capture_valid === true ? "" : "disabled"}>Stop &amp; Save</button>` : `<button data-stop="${this.attr(device.device_id)}">Stop</button>`) : training.status === "active" ? `<button data-stop="${this.attr(device.device_id)}">Stop</button>` : manual ? `<button data-manual="${this.attr(device.device_id)}">${training.status === "complete" ? "Retrain Manual" : "Manual Training"}</button>` : `<button data-quick="${this.attr(device.device_id)}">Quick ON/OFF</button><button data-full="${this.attr(device.device_id)}">Full Cycle</button>`;
    ...
    bindActionButtons() {
      this.querySelectorAll("[data-quick]").forEach(b => b.onclick = () => this.startTraining(b.dataset.quick, "quick")); this.querySelectorAll("[data-full-action]").forEach(b => b.onclick = () => this.fullCycleAction(b.dataset.fullAction, b.dataset.fullOp));
    ...
    trainingLabel(training) {
      const status = training?.status || "not trained", observations = this.quickObservationText(training), result = training?.result || {}, learned = Number(training?.learned_signature?.load_w ?? result?.peak_delta_w), learnedText = Number.isFinite(learned) && learned > 0 ? ` · Load: ${learned.toFixed(0)} W` : "";
      ...
      if (status === "complete") return observations ? `Complete · ${observations}${learnedText}` : `Complete${learnedText}`;
    ...
    row(device, selected) {
      ...
      return `<tr data-device-id="${this.attr(device.device_id)}"><td><input class="monitor" type="checkbox" data-monitor="${this.attr(device.device_id)}" ${monitored ? "checked" : ""}></td><td><input class="train" type="checkbox" data-train="${this.attr(device.device_id)}" ${trainChecked ? "checked" : ""} ${monitored ? "" : "disabled"}></td><td><strong>${this.escape(device.name || device.device_id)}</strong><small>${this.escape(device.category || device.model || "")}</small></td><td>${this.escape(device.area || "")}</td><td>${manual ? "Manual" : "HA"}</td><td class="power-cell ${fullCaptureValid ? "capture-valid" : ""}" title="${fullCaptureValid ? `Capture valid · stable load ${Number(result.stable_load_w || liveDelta).toFixed(0)} W` : ""}">${Number.isFinite(power) ? `${power.toFixed(0)} W` : "—"}</td><td class="state-cell">${this.renderState(device)}</td><td class="training">${this.escape(this.trainingLabel(training))}</td><td class="method-cell">${this.escape(training.method || "—")}</td><td class="actions">${monitored ? action : ""}</td></tr>`;
    }
    ...
    bind() {
      ...
      this.querySelectorAll("[data-train]").forEach(box => box.addEventListener("change", event => { const id = event.currentTarget.dataset.train; if (event.currentTarget.checked) this.trainSelected.add(id); else this.trainSelected.delete(id); const bulkButton = this.querySelector("#bulk-train"); if (bulkButton) { bulkButton.disabled = this.trainSelected.size === 0; bulkButton.textContent = `Auto Train Selected (${this.trainSelected.size})`; } }));
      this.querySelectorAll("[data-monitor]").forEach(box => box.addEventListener("change", event => { const selected = this.getSelectedIds(), id = event.currentTarget.dataset.monitor, checked = event.currentTarget.checked; if (checked) selected.add(id); else selected.delete(id); this.pending = selected; const row = event.currentTarget.closest("tr"), trainBox = row?.querySelector("[data-train]"); if (trainBox) trainBox.disabled = !checked; }));
      this.querySelectorAll("[data-quick]").forEach(b => b.addEventListener("click", () => this.startTraining(b.dataset.quick, "quick"))); this.querySelectorAll("[data-full-action]").forEach(b => b.addEventListener("click", () => this.fullCycleAction(b.dataset.fullAction, b.dataset.fullOp))); this.querySelectorAll("[data-full]").forEach(b => b.addEventListener("click", () => this.startTraining(b.dataset.full, "full_cycle"))); this.querySelectorAll("[data-manual]").forEach(b => b.addEventListener("click", () => this.startTraining(b.dataset.manual, "manual"))); this.querySelectorAll("[data-stop]").forEach(b => b.addEventListener("click", () => this.stopTraining(b.dataset.stop)));
    }
