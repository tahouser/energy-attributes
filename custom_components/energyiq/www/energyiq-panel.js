...
(() => {
  ...
  class EnergyIQPanel extends HTMLElement {
    ...
    render() {
      ...
      const trainedCount = devices.filter(d => d.training?.status === "complete").length;
      const excludedCount = devices.filter(d => d.classification !== "monitor").length;
      const visible = devices.filter(d => {
        if (this.view === "all") return true;
        if (this.view === "excluded") return d.classification !== "monitor";
        return d.training?.status === "complete";
      }).sort((a, b) => String(a.name).localeCompare(String(b.name)));
      this.innerHTML = `<style>${this.styles()}</style><div class="shell"><header><div class="title-block"><div class="title-row"><button id="back" class="back-button" title="Back to Home Assistant" aria-label="Back to Home Assistant">← Back</button><img class="brand-logo" src="${this.brandUrl}" alt="EnergyIQ"><div class="brand-copy"><div class="brand-name-row"><h1>EnergyIQ</h1><span class="tagline">Know your power</span></div><p>Whole-home electrical intelligence · v${this.workspace?.version || "?"}</p></div></div></div></header><div id="summary-area">${this.renderSummary()}</div>${this.trainingWorkspaceOpen ? `<div id="training-area">${this.renderTrainingWorkspace()}</div>` : ""}<div class="toolbar"><div class="views"><button id="trained" class="${this.view === "trained" ? "selected" : ""}">Trained (${trainedCount})</button><button id="all" class="${this.view === "all" ? "selected" : ""}">All (${devices.length})</button><button id="excluded" class="${this.view === "excluded" ? "selected" : ""}">Excluded (${excludedCount})</button></div><div class="toolbar-actions"><button id="add">＋ Add Device / Entity</button><button id="save" class="primary">Save Monitoring</button></div></div><div class="table-scroll"><table><thead><tr><th>Monitor</th><th class="train-header ${this.trainSelected.size ? "has-selection" : ""}" id="train-header" title="${this.trainSelected.size ? "Open Training for selected devices" : "Select devices, then open Training"}">Train${this.trainSelected.size ? ` (${this.trainSelected.size})` : ""}</th><th>Device</th><th>Area</th><th>Source</th><th>Power</th><th>State</th><th>Training</th><th>Method</th></tr></thead><tbody>${visible.length ? visible.map(d => this.row(d)).join("") : `<tr><td colspan="9" class="empty">No loads in this view.</td></tr>`}</tbody></table></div></div>`;
      this.bind();
    }
    ...
    async startWorkspaceTraining(retrain = false) {
      ...
      try {
        ...
        if (this.trainingComplete(completedId) && remainingQueue.length === 0) {
          this.trainSelected.delete(completedId);
          this.trainingWorkspaceOpen = false;
          this.trainingQueue = [];
          this.activeTrainingId = null;
          this.view = "trained";
