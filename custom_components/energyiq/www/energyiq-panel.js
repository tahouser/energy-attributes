...
(() => {
  ...
  class EnergyIQPanel extends HTMLElement {
    constructor() {
      ...
      this.workspace = null;
      this.bulk = null;
      this.brandUrl = "/energyiq-brand/icon.png?v=31423";
      this.view = "all";
    ...
    toggleUpperSections() {
      this.upperSectionsCollapsed = !this.upperSectionsCollapsed;
      this.render();
    }
    ...
    sortHeader(column,label) {
      ...
      return '<th class="sortable-header '+(active?"sort-active":"")+'"><button type="button" class="sort-button" data-sort="'+column+'" aria-label="Sort by '+label+'">'+label+arrow+'</button></th>';
    }
hasPendingChanges() {
    ...
    render() {
      ...
      this.innerHTML=`<style>${this.styles()}</style><div class="shell">
        <header><div class="title-block"><div class="title-row"><button id="back" class="back-button" title="Back to Home Assistant" aria-label="Back to Home Assistant"><svg viewBox="0 0 72 48" aria-hidden="true"><path d="M4 24H66M4 24L24 4M4 24L24 44" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg></button><div class="brand-logo" aria-label="EnergyIQ"><svg viewBox="0 0 128 128" role="img" aria-hidden="true"><circle cx="64" cy="64" r="54" fill="none" stroke="#24d7a1" stroke-width="7"/><path d="M30 58 64 31l34 27v37a6 6 0 0 1-6 6H36a6 6 0 0 1-6-6Z" fill="none" stroke="#fff" stroke-width="7" stroke-linejoin="round"/><path d="M69 42 52 70h15l-8 22 22-31H66Z" fill="#fff"/></svg></div><div class="brand-copy"><div class="brand-name-row"><h1>EnergyIQ</h1><span class="tagline">Know your power</span></div><p>Whole-home electrical intelligence · v${UI_VERSION}</p></div></div></div></header>
        <div class="upper-config-toggle-wrap">
          <button id="toggle-upper-sections" class="upper-config-toggle" type="button"
            aria-expanded="${this.upperSectionsCollapsed ? "false" : "true"}"
            aria-controls="upper-config">
            <span class="upper-config-chevron" aria-hidden="true">${this.upperSectionsCollapsed ? "▸" : "▾"}</span>
...
${selected ? `<div class="selection-bar"><div class="selection-count"><span class="selection-box">☐</span><strong>${selected}</strong> selected</div><div class="selection-actions">${selected ? `<button id="begin-training" class="action-training" ${canTrain?"":"disabled"}>Begin Training</button>` : ""}<button id="include" class="action-include" ${selected?"":"disabled"}>Include</button>${this.view === "excluded" ? `<button id="remove-excluded" class="action-exclude" ${selected?"":"disabled"}>Remove Selected</button>` : `<button id="exclude" class="action-exclude" ${selected?"":"disabled"}>Exclude</button>`}<button id="clear-selection" ${selected?"":"disabled"}>Clear Selection</button></div></div>` : ""}\n        <div class="table-scroll"><table><thead><tr><th class="select-col">☐</th>${this.sortHeader("name","Device")}${this.sortHeader("area","Area")}${this.sortHeader("source","Source")}${this.sortHeader("live","Live Watts")}${this.sortHeader("trained","Trained Watts")}${this.sortHeader("state","State")}${this.sortHeader("training","Training")}${this.sortHeader("method","Method")}</tr></thead><tbody>${rows.length?rows.map(d=>this.row(d)).join(""):`<tr><td colspan="9" class="empty">No devices match this view.</td></tr>`}</tbody></table></div>
    ...
    historyEntityFor(device) {
      const measurements = Array.isArray(device?.measurements) ? device.measurements : [];
      const power = measurements.find(item => item?.kind === "power" && item?.entity_id);
      if (power?.entity_id) return power.entity_id;
      const firstMeasurement = measurements.find(item => item?.entity_id);
      if (firstMeasurement?.entity_id) return firstMeasurement.entity_id;
      const controls = Array.isArray(device?.controls) ? device.controls : [];
      return controls.find(item => item?.entity_id)?.entity_id || null;
    }
    showEntityHistory(deviceId) {
      const device = this.getDevice(deviceId);
      const entityId = this.historyEntityFor(device);
      if (!entityId) {
    ...
    bind() {
      ...
      this.querySelector("#clear-selection")?.addEventListener("click",()=>{this.selectedIds.clear();this.render();});
            this.querySelectorAll(".sort-button[data-sort]").forEach(button=>button.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();this.setSort(button.dataset.sort);}));
this.querySelector("#begin-training")?.addEventListener("click",()=>this.beginSelectedTraining());
    ...
    styles() { return `
      ...
      .table-scroll{overflow:auto;min-height:0;flex:1;border:1px solid var(--divider-color);border-radius:11px;overscroll-behavior:none;touch-action:pan-x pan-y}table{border-collapse:collapse;width:100%;min-width:900px;background:var(--card-background-color)}th,td{padding:10px 11px;border-bottom:1px solid var(--divider-color);text-align:left;vertical-align:middle}th{position:sticky;top:0;background:var(--card-background-color);z-index:1;font-size:11px;color:var(--secondary-text-color);text-transform:uppercase;letter-spacing:.05em;white-space:nowrap}.sort-button{display:block;width:100%;border:0;border-radius:0;background:transparent;color:inherit;padding:0;text-align:left;font:inherit;cursor:pointer}.sort-button:hover{color:var(--primary-text-color)}.sortable-header.sort-active,.sortable-header.sort-active .sort-button{color:#5dd5ff}.select-col{width:44px;text-align:center}td small{display:block;color:var(--secondary-text-color);font-size:11px;margin-top:2px}.select-device{width:21px;height:21px;margin:0;accent-color:#36c8ff}.device-name-cell{min-width:190px}.entity-history-link{display:block;width:100%;padding:0;border:0;border-radius:4px;background:transparent;color:var(--primary-text-color);text-align:left;font:inherit}.entity-history-link:hover{background:rgba(54,200,255,.08);color:#5dd5ff;transform:none}.entity-history-link strong{font-weight:700}.power-cell,.trained-power-cell{white-space:nowrap;font-variant-numeric:tabular-nums}.method-cell{text-transform:capitalize;color:var(--secondary-text-color)}.state-cell{text-align:center;width:70px}.state-box{display:inline-flex;align-items:center;justify-content:center;min-width:46px;height:23px;padding:0 8px;border-radius:12px;font-size:11px;font-weight:800}.state-box.on{background:#35b85a;color:#07140b}.state-box.off{background:rgba(255,255,255,.08);color:var(--secondary-text-color);border:1px solid var(--divider-color)}.state-box.unknown{background:rgba(255,77,95,.12);color:#ff6574}.training-dot{display:inline-block;width:15px;height:15px;border-radius:50%;vertical-align:middle}.training-dot{display:none}.training-box{display:inline-flex;width:46px;height:23px;border-radius:12px;box-sizing:border-box;vertical-align:middle}.training-box.trained{background:rgba(53,184,90,.28);border:1px solid rgba(53,184,90,.55)}.training-box.active,.training-box.error{background:rgba(255,77,95,.28);border:1px solid rgba(255,77,95,.58)}.training-box.untrained{background:rgba(255,255,255,.07);border:1px solid var(--divider-color)}
      ...
      @media(max-width:1100px){.summary{grid-template-columns:repeat(3,1fr)}.meter-grid{grid-template-columns:repeat(2,1fr)}.table-scroll{max-height:none}.queue-strip{grid-template-columns:repeat(3,1fr)}.queue-more{grid-column:1/-1}.training-grid{grid-template-columns:1fr}}@media(max-width:700px){.upper-config-toggle-wrap{display:flex;justify-content:flex-end;margin:-3px 0 5px}.upper-config-toggle{min-height:28px}.upper-config.collapsed{display:none}header:before,header:after{display:none!important}.title-row{display:grid;grid-template-columns:34px 46px minmax(0,1fr);align-items:center;gap:9px;padding:0 2px}.back-button{grid-column:1;width:52px;height:34px;flex:none;align-self:center;padding:0;border:0;border-radius:0;background:transparent!important;box-shadow:none;color:var(--primary-text-color);display:grid;place-items:center}.back-button svg{width:52px;height:34px}.back-button:hover,.back-button:focus,.back-button:active{background:transparent!important;box-shadow:none}.brand-logo{grid-column:2;width:46px;height:46px;flex:none}.brand-logo svg{width:46px;height:46px;display:block}.brand-copy{grid-column:3;min-width:0;flex:1}.brand-name-row{display:flex;gap:9px;flex-wrap:nowrap;align-items:baseline;min-width:0}.brand-name-row h1{font-size:25px;line-height:1;margin:0;flex:none}.tagline{display:block;font-size:16px;line-height:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0}.brand-copy p{font-size:12px;line-height:1.15;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.meter-grid{grid-template-columns:1fr}.section-heading{align-items:flex-start;flex-direction:column}.shell{padding:92px 12px 16px}header{margin:0;padding:12px 14px 13px}.summary{grid-template-columns:repeat(2,1fr);gap:7px}.metric{padding:10px}.metric strong{font-size:17px}.toolbar{align-items:stretch}.views,.toolbar-actions{width:100%}.views button,.toolbar-actions button{flex:1}.list-tools{align-items:stretch}.search-box{max-width:none}.result-count{display:none}.selection-bar{align-items:stretch}.selection-actions{width:100%}.selection-actions button{flex:1}.table-scroll{max-height:none}.queue-strip{grid-template-columns:1fr}.training-workspace{padding:12px}.capture-grid{grid-template-columns:repeat(2,1fr)}.training-head{align-items:center}.training-head h2{font-size:20px}.modal-actions{grid-template-columns:1fr 1fr}.modal-actions span{display:none}}
    `; }
  }
