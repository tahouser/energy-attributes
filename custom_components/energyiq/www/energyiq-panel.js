...
(() => {
  ...
  class EnergyIQPanel extends HTMLElement {
    ...
    sortDevices(devices) {
      const direction = this.sortDirection === "desc" ? -1 : 1, column = this.sortColumn || "name";
      return devices.map((device,index)=>({device,index,value:this.sortValue(device,column)})).sort((a,b)=>{
        const av=a.value,bv=b.value,an=typeof av==="number"&&Number.isFinite(av),bn=typeof bv==="number"&&Number.isFinite(bv);
        let result=0;
    ...
    sortHeader(column,label) {
      const active=this.sortColumn===column, arrow=active?(this.sortDirection==="asc"?" ↑":" ↓"):"";
