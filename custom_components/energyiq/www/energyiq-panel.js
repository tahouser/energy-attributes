  ...
  class EnergyIQPanel extends HTMLElement {
    ...
    render() {
      ...
      this.innerHTML=`<style>${this.styles()}</style><div class="shell">
        ...
        <div class="list-tools"><label class="search-box"><span>⌕</span><input id="device-search" type="search" value="${this.escape(this.searchTerm)}" placeholder="Search devices, areas, entities…" autocomplete="off"></label><span class="result-count">${all.length} ${all.length===1?"entity":"entities"}</span></div>
