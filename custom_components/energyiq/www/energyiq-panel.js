...
(() => {
  ...
  class EnergyIQPanel extends HTMLElement {
    ...
    renderBulk() {
      const b = this.bulk || { status: "idle" };
      if (b.status !== "running") return "";
      const total = Number(b.total || 0);
      const completed = Number(b.completed || 0);
      const current = Number(b.current_index || 0);
      const percent = total ? Math.min(100, Math.round((completed / total) * 100)) : 0;
      const currentDevice = this.getDevices().find(d => d.device_id === b.current_device_id);
    ...
    bind() {
      ...
      this.querySelectorAll("[data-manual]").forEach(b => b.addEventListener("click", () => this.startTraining(b.dataset.manual, "manual")));
      this.querySelectorAll("[data-stop]").forEach(b => b.addEventListener("click", () => this.stopTraining(b.dataset.stop)));
    }
    ...
    async startTraining(deviceId, method) {
      const device = this.getDevices().find(d => d.device_id === deviceId);
      if (!device) return;
      const message = method === "quick" ? `EnergyIQ will automatically turn “${device.name}” ON and OFF during Quick Training. Continue?` : method === "manual" ? `Manual training for “${device.name}”. No command will be sent to the device. Continue?` : `Run a Full Cycle training session for “${device.name}”?`;
    ...
    async openAddDialog() {
      ...
      } catch (error) { this.showToast(error?.message || "Unable to load Home Assistant entities", true); }
    }

    showAddDialog(entities) {
      const backdrop = document.createElement("div");
      backdrop.className = "modal-backdrop";
      backdrop.innerHTML = `<div class="modal" role="dialog" aria-modal="true"><div class="modal-head"><div><h2>Add Device / Entity</h2><p>Search the complete Home Assistant entity registry.</p></div><button id="close">×</button></div>
        <div class="search-row"><input id="search" type="search" placeholder="Search name, entity ID, domain, state…" autocomplete="off"><select id="domain"><option value="">All domains</option></select></div>
        <select id="choices" size="11"></select><div id="ownership" class="ownership">Select an entity to see its EnergyIQ association.</div>
        <div class="modal-actions"><button id="manual">Add Manual Device</button><span></span><button id="cancel">Cancel</button><button id="add-selected" class="primary">Add Entity</button></div></div>`;
