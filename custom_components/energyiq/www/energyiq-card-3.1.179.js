/* EnergyIQ dashboard card — 3.1.179 */
  ...
  class EnergyIQCard extends HTMLElement {
    ...
    _historyValueAt(states,timeMs){
      ...
      for(const item of (states||[])){
        ...
        const n=Number(item.s??item.state);
        if(Number.isFinite(n))value=n;
      }
      return value;
    }
    _periodCost(states,start,end,sampleEnd=end){
      const a=this._historyValueAt(states,start.getTime());
      const b=this._historyValueAt(states,Math.min(end.getTime(),sampleEnd.getTime()));
    ...
    async _loadCostHistory(){
      ...
      try{
        ...
        const now=new Date();
        const currentStart=this._periodStart(period,0,now);
        const oldest=this._periodStart(period,-30,now);
        ...
        const history=await this._ws({type:"history/history_during_period",start_time:oldest.toISOString(),end_time:now.toISOString(),entity_ids:ids,include_start_time_state:true,significant_changes_only:false,minimal_response:true,no_attributes:true});
        const peakStates=peakId?(history?.[peakId]||[]):[];
        const offStates=offId?(history?.[offId]||[]):[];
        if(peakId&&!peakStates.length)throw new Error("Peak cost sensor returned no history.");
        if(offId&&!offStates.length)throw new Error("Off-Peak cost sensor returned no history.");
        const peak=this._learnCost(peakStates,period,now,30);
        const off=this._learnCost(offStates,period,now,30);
    ...
    _brandIcon(){return '<svg viewBox="0 0 64 64" aria-hidden="true" focusable="false"><circle cx="32" cy="32" r="28.5" fill="rgba(28,205,170,.10)" stroke="#2d8f7b" stroke-width="3"/><path d="M15 30.5 32 15l17 15.5v18H15z" fill="none" stroke="#e7ecef" stroke-width="3.2" stroke-linejoin="round"/><path d="M36 14 24.5 34h8l-5 16L43 28H35z" fill="#ffd447" stroke="#f4bd22" stroke-width="1.2" stroke-linejoin="round"/></svg>';}
    ...
    _cost(total,peak,off){
      ...
      const bar=(label,item)=>{
        const value=Number(item?.current)||0,target=Number(item?.average),scale=Number.isFinite(target)&&target>0?target*1.10:null;
        const width=scale?Math.max(0,Math.min(100,value/scale*100)):(value>0?100:0);
