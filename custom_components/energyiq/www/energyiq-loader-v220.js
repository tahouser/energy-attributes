/* EnergyIQ v2.2.0 frontend loader. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=220`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub) sub.textContent = "Whole-home electrical intelligence · v2.2.0";
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched220) {
      ...
      proto.__energyiqVersionPatched220 = true;
    }
  }
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver220) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver220 = observer;
  }
