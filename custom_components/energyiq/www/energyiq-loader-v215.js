/* EnergyIQ v2.1.5 frontend loader. Uses a unique URL to force a fresh browser fetch. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=215`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub && sub.textContent !== "Whole-home electrical intelligence · v2.1.5") {
      sub.textContent = "Whole-home electrical intelligence · v2.1.5";
    }
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched215) {
      ...
      proto.__energyiqVersionPatched215 = true;
    }
  }
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver215) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver215 = observer;
  }
