...
(() => {
  ...
  const install = () => {
    ...
    Panel.prototype._energyiqAddStateColumn = function () {
      ...
      const selectedIds = this._pendingSelections || new Set(
        devices.filter(x => x.classification === "monitor").map(x => x.device_id)
      );
      const visible = devices.filter(x => this._listMode === "excluded" ? !selectedIds.has(x.device_id) : selectedIds.has(x.device_id));
      const ordered = [...visible].sort((a,b) => {
        const an=!!a.manual_added,bn=!!b.manual_added;
        if(an!==bn)return an?-1:1;
        const am=String(a.source||"").toLowerCase()==="manual",bm=String(b.source||"").toLowerCase()==="manual";
      ...
      rows.forEach((row, index) => {
        ...
        td.className = "energyiq-state-cell";
        td.innerHTML = this._energyiqStateHtml(device);
    ...
    Panel.prototype._energyiqEntityState = function (device) {
      ...
      if (!hass || !device) return {kind:"unknown", label:"Unknown"};
    ...
    Panel.prototype._energyiqStateHtml = function (device) {
      const state = this._energyiqEntityState(device);
      const title = state.kind === "on" ? "Entity is ON" : state.kind === "off" ? "Entity is OFF" : "Entity state unavailable";
      return `<span class="energyiq-state ${state.kind}" title="${title}" aria-label="${state.label}"><span class="energyiq-state-dot"></span><span>${state.label}</span></span>`;
    };
    Panel.prototype._energyiqRefreshStates = function () {
      if (!this.isConnected) return;
      const rows = [...this.querySelectorAll(".table-wrap table tr")].slice(1);
      const devices = this.data?.devices || [];
      ...
      rows.forEach((row,index) => {
        const cell = row.querySelector(".energyiq-state-cell");
        if (cell && ordered[index]) cell.innerHTML = this._energyiqStateHtml(ordered[index]);
      });
    };
    const style = document.createElement("style");
    style.textContent = `.energyiq-state{display:inline-flex;align-items:center;gap:7px;font-weight:600;white-space:nowrap}.energyiq-state-dot{width:13px;height:13px;border-radius:50%;display:inline-block;box-sizing:border-box;border:1px solid var(--divider-color)}.energyiq-state.on .energyiq-state-dot{background:#2e7d32;border-color:#2e7d32}.energyiq-state.off .energyiq-state-dot{background:#c62828;border-color:#c62828}.energyiq-state.unknown{color:var(--secondary-text-color);font-weight:400}`;
