...
if (!customElements.get(TAG)) {
  class EnergyIQCard extends HTMLElement {
    constructor() { super(); this._hass=null; this._cfg={}; this._data=null; this._entryId=null; this._view=0; this._timer=null; this._busy=false; this._ro=null; this._click=this._handleClick.bind(this); this._activeLoadsOpen=false; this._dashboardHistory=null; this._dashboardHistoryAt=0; this._costPeriod="day"; this._costHistory=null; this._costHistoryAt=0; this._costLearned=null; this._costSwipeStartX=0; this._costSwipeStartY=0; this._costSwipeActive=false; }
    ...
    async _refresh(){try{this._data=await this._ws({type:"energy_attribution/workspace",entry_id:this._entryId});if(this._view===2&&Date.now()-this._costHistoryAt>30000)await this._loadCostHistory();if(this._view===0&&Date.now()-this._dashboardHistoryAt>30000)await this._loadConsumptionHistory();this._render();}catch(e){console.error("EnergyIQ card",e);}}
    ...
    _historyNumberAt(states,time,preferAfter){
      ...
      for(const item of states||[]){
        ...
        const delta=t-time;        if(preferAfter ? delta>=0 : delta<=0){          const distance=Math.abs(delta);          if(distance<bestDelta){best=item;bestDelta=distance;}
        }
      }
    ...
    async _statChangeForPeriod(statisticId,start,end){      if(!statisticId||!start||!end)return null;
      try{        const result=await this._ws({
          type:"recorder/statistic_during_period",          statistic_id:statisticId,
      ...
      try{
        ...
        const [stats,history,peakColorHistory]=await Promise.all([
          ...
          peakColorEntity?this._ws({
            type:"history/history_during_period",            start_time:oldest.toISOString(),            end_time:now.toISOString(),
    ...
    async _loadConsumptionHistory(){
      ...
      try{
        ...
        const build=async(id)=>{
          ...
          const average=samples.length?samples.reduce((a,b)=>a+b,0)/samples.length:null;
          const max=Math.max(current,...samples,0);
    ...
    _bindCostSwipe(){
      ...
      el.addEventListener("touchend",event=>{        if(!this._costSwipeActive)return;
    ...
    _render(){
      ...
      var titles=["CONSUMPTION","MYSTERY WATTS","COST"],subs=["Current attributed power","Known vs. unexplained power",""],body=this._view===0?this._consumption():this._view===1?this._mystery(home,known,mystery):this._cost(total,peak,off);
      ...
      const consumptionTotal=this._dashboardHistory?.total;
      const consumptionMoney=Number.isFinite(Number(consumptionTotal))?Number(consumptionTotal).toFixed(1)+" kWh":"—";
      const headExtra=this._view===2?'<div class="cost-head-total"><span>'+costLabel+'</span><strong>'+costMoney+'</strong></div>':this._view===0?'<div class="cost-head-total consumption-head-total"><span>'+costLabel+'</span><strong>'+consumptionMoney+'</strong></div>':"";
    ...
    _consumption(){
      ...
      const money=v=>Number(v||0).toFixed(1)+" kWh";
      if(!h)return '<div class="cost-loading">Loading consumption history…</div>';
      const bar=(label,item)=>{
        const value=Number(item?.current)||0;
        const width=this._periodProgress(this._costPeriod)*100;
        const color=item?.color||"green";
        return '<div class="cost-bar-row consumption-bar-row"><div class="cost-bar-head"><span>'+label+'</span><strong>'+money(value)+'</strong></div><div class="cost-bar-track"><div class="consumption-bar-fill '+color+'" style="width:'+width.toFixed(2)+'%"></div></div></div>';
      };
      return '<div class="cost-swipe consumption-swipe" role="group" aria-label="Consumption period '+periodLabel+'. Swipe left or right to change period."><div class="cost-bars">'+bar("Peak",h.peak)+bar("Off-Peak",h.off)+'</div><div class="cost-period-nav"><button type="button" data-cost-period="prev" aria-label="Previous consumption period">‹</button><strong>'+periodLabel+'</strong><button type="button" data-cost-period="next" aria-label="Next consumption period">›</button></div></div>';
    }
