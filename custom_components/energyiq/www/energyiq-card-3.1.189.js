/* EnergyIQ dashboard card — 3.1.189 */
...
if (!customElements.get(TAG)) {
  class EnergyIQCard extends HTMLElement {
    ...
    _segmentColor(ratio){
      return ratio==null?"neutral":ratio>1.10?"red":ratio>=0.90?"yellow":"green";
    }
    ...
    _segmentHistoricalSamples(states,period,currentPeriodStart,segmentStart,segmentEnd,now,count=30){
      ...
      for(let i=1;i<=count;i++){
        ...
        const b=new Date(Math.min(a.getTime()+segmentMs,priorEnd.getTime()));
        if(a>=priorEnd||b<=a)continue;
        const v=this._periodCost(states,a,b,b);
        if(v!=null&&Number.isFinite(v)&&v>=0)samples.push(v);
      }
      if(!samples.length){
        const clockMinutes=segmentStart.getHours()*60+segmentStart.getMinutes();
        for(let d=1;d<=30;d++){
    ...
    _buildSegmentProfile(states,period,now,count=30){
      ...
      const segments=windows.map((w,index)=>{
        ...
        const average=samples.length?samples.reduce((a,b)=>a+b,0)/samples.length:null;
        const ratio=average>0&&current!=null?current/average:null;
        return {index,start:w.start,end:w.end,current,average,ratio,color:this._segmentColor(ratio),samples:samples.length,visible:visibleMs>0,visibleFraction:Math.max(0,Math.min(1,visibleMs/Math.max(1,w.end.getTime()-w.start.getTime())))};
      });
