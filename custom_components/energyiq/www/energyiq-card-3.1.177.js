/* EnergyIQ dashboard card — 3.1.177 */
  ...
  class EnergyIQCard extends HTMLElement {
    ...
    async _loadCostHistory(){
      ...
      try{
        ...
        if(!powerId)throw new Error("EnergyIQ whole-home power entity is unavailable.");
        const end=new Date(),start=new Date(end);
        start.setDate(start.getDate()-30);start.setHours(0,0,0,0);
        const stats=await this._ws({type:"recorder/statistics_during_period",start_time:start.toISOString(),end_time:end.toISOString(),statistic_ids:[powerId],period:"5minute",types:["mean","state"]});
        const rows=(stats&&stats[powerId])||[];
        ...
        if(ids.length){
          const {start:periodStart,end:periodEnd}=this._periodBounds(this._costPeriod);
          const currentHistory=await this._ws({type:"history/history_during_period",start_time:periodStart.toISOString(),end_time:periodEnd.toISOString(),entity_ids:ids,include_start_time_state:true,significant_changes_only:false,minimal_response:true,no_attributes:true});
          const delta=id=>{
            if(!id)return null;
            const states=(currentHistory&&currentHistory[id])||[];
            const nums=states.map(x=>Number(x.s??x.state)).filter(Number.isFinite);
            const current=this._state(id),first=nums.length?nums[0]:null,last=nums.length?nums[nums.length-1]:current;
          ...
          if(peakEnergyId){
            try{
              const ps=await this._ws({type:"recorder/statistics_during_period",start_time:periodStart.toISOString(),end_time:periodEnd.toISOString(),statistic_ids:[peakEnergyId],period:"5minute",types:["change","state"]});
    ...
    _brandIconUrl(){return "/energyiq-brand/icon.png?v=31450";}
