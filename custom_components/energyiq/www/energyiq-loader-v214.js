/* EnergyIQ v2.1.4 frontend loader. Uses a unique URL to force a fresh browser fetch. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=214`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub && sub.textContent !== "Whole-home electrical intelligence · v2.1.4") {
      sub.textContent = "Whole-home electrical intelligence · v2.1.4";
    }
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched214) {
      ...
      proto.__energyiqVersionPatched214 = true;
    }
  }
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver214) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver214 = observer;
  }
