  ...
  class EnergyIQCard extends HTMLElement {
    ...
    _cost(total,peak,off){
      ...
      const bar=(value,info,type)=>{
        ...
        // Keep visible headroom so the colored fill never runs into the running-total box.
        // When actual usage exceeds the learned target, the scale expands automatically.
        const scale=target==null
          ? Math.max(0.01,value*1.15)
          : Math.max(0.01,target*1.10,value*1.10);
        const pct=Math.max(0,Math.min(92,value/scale*100));
        const ratio=target==null?0:value/target;
        const status=ratio<0.75?"normal":ratio<0.90?"elevated":ratio<1.00?"high":ratio<1.15?"over":"critical";
        return '<div class="cost-bar-row"><div class="cost-bar-label"><span>'+(type==="peak"?"Peak":"Off-Peak")+'</span></div><div class="cost-bar-main"><div class="cost-track"><div class="cost-fill '+type+' '+status+'" style="width:'+pct+'%"></div></div><div class="cost-total">'+money(value)+'</div></div></div>';
      };
