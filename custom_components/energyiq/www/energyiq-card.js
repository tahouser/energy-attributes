/* EnergyIQ dashboard card — 3.1.170 */
const TAG = "energyiq-card";
const BRAND_ICON_URL = "/energyiq-brand/icon.png?v=31443";
if (!customElements.get(TAG)) {
  class EnergyIQCard extends HTMLElement {
    ...
    _costLearningSpec(period){
      if(period==="week")return {count:12};
      if(period==="month")return {count:12};
      return {count:30};
    }
    ...
    _learnedBar(value,samples){
      ...
      if(!clean.length||!Number.isFinite(value))return {pct:value>0?50:0,target:null,targetPct:92.6,sampleCount:clean.length};
      // Moving target: historical maximum at the same elapsed point.
      const target=Math.max(0.01,...clean);
      const ratio=Math.max(0,value/target);
      // Keep a small amount of track beyond target for >100% readings.
      const pct=Math.max(3,Math.min(100,ratio*92.6));
      return {pct,target,ratio,targetPct:92.6,sampleCount:clean.length};
    }
      ...
      try{
        ...
        try{
          ...
          if(rows.length){
            ...
            const lastRow=rows[rows.length-1],lastState=Number(lastRow.state),current=this._state(peakEnergyId);
            let tail=0;
        ...
        // Compare current accumulated cost with prior periods at the same elapsed point.
    ...
    _cost(total,peak,off){
      ...
      const bar=(value,info,type)=>{
        ...
        const marker=info&&Number.isFinite(info.targetPct)?info.targetPct:92.6;
        return '<div class="cost-bar-row"><div class="cost-bar-label"><span>'+(type==="peak"?"Peak":"Off-Peak")+'</span></div><div class="cost-bar-main"><div class="cost-track"><div class="cost-fill '+type+'" style="width:'+pct+'%"></div><div class="cost-target-marker" style="left:'+marker+'%"></div></div><div class="cost-total">'+money(value)+'</div></div></div>';
      };
