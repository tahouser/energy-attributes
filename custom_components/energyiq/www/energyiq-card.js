/* EnergyIQ dashboard card — 3.1.152 */
const TAG = "energyiq-card";
if (!customElements.get(TAG)) {
  class EnergyIQCard extends HTMLElement {
    constructor() { super(); this._hass=null; this._cfg={}; this._data=null; this._entryId=null; this._view=0; this._timer=null; this._busy=false; this._ro=null; this._click=this._handleClick.bind(this); this._costPeriod="day"; this._costHistory=null; this._costHistoryAt=0; }
    ...
    _handleClick(e){this._next(e);const period=e.target.closest&&e.target.closest("[data-cost-period]");if(period){e.stopPropagation();const p=period.getAttribute("data-cost-period");if(p&&p!==this._costPeriod){this._costPeriod=p;this._costHistory=null;this._costHistoryAt=0;this._loadCostHistory();}return;}const active=e.target.closest&&e.target.closest("[data-active-loads]");if(active){e.stopPropagation();this._showActiveLoads();return;}const open=e.target.closest&&e.target.closest("[data-open-energyiq]");if(open){e.stopPropagation();this._openEnergyIQ();return;}const close=e.target.closest&&e.target.closest("[data-close-active]");if(close){e.stopPropagation();this._closeActiveLoads();return;}}
    ...
    _render(){
      ...
      var titles=["CONSUMPTION","MYSTERY WATTS","COST"], subs=["Current attributed power","Known vs. unexplained power",this._costPeriod==="hour"?"This hour":this._costPeriod==="month"?"This month":"Today"], body=this._view===0?this._pareto(devices):this._view===1?this._mystery(home,known,mystery):this._cost(total,peak,off);
