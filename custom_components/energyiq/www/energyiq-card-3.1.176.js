...
if (!customElements.get(TAG)) {
  class EnergyIQCard extends HTMLElement {
    ...
    _percentile(values,value){
      const sorted=(values||[]).filter(Number.isFinite).sort((a,b)=>a-b);
      if(!sorted.length||!Number.isFinite(value))return null;
      let below=0;
      for(const v of sorted)if(v<=value)below++;
      return Math.max(0,Math.min(100,(below-0.5)/sorted.length*100));
    }
    _learnedBar(value,samples){
      const clean=(samples||[]).filter(v=>Number.isFinite(v)&&v>=0);
      if(!clean.length||!Number.isFinite(value))return {pct:value>0?50:0,target:null,ratio:null,color:"green",sampleCount:clean.length};
      // Use the average of up to 30 prior matching periods at the same elapsed point.
      const average=clean.reduce((sum,v)=>sum+v,0)/clean.length;
      const ratio=average>0?Math.max(0,value/average):value>0?Infinity:0;
    ...
    _resolveEntity(preferred,patterns){
      ...
      for(const [id,obj] of Object.entries(states)){
        ...
        let score=0;
        for(const p of (patterns||[]))if(text.includes(p))score+=1;
        if(score)scored.push({id,score});
      }
      scored.sort((a,b)=>b.score-a.score||a.id.localeCompare(b.id));
      return scored[0]?.id||preferred||null;
    }
    _statisticsEnergy(rows,startMs,endMs){
      if(!Array.isArray(rows)||endMs<=startMs)return 0;
      let total=0;
      for(let i=0;i<rows.length;i++){
        const row=rows[i]||{};
        const t=Number(row.start);
        if(!Number.isFinite(t))continue;
        const next=i+1<rows.length?Number(rows[i+1]?.start):endMs;
        const a=Math.max(startMs,t), b=Math.min(endMs,Number.isFinite(next)?next:endMs);
        const mean=Number(row.mean);
        if(b>a&&Number.isFinite(mean))total+=this._powerValue(mean)*(b-a)/3600000;
      }
      return Math.max(0,total);
    }
    _timelineFromStatistics(rows){
      const now=new Date(),today=this._localDayStart(now),elapsed=Math.max(0,now.getTime()-today.getTime());
      const currentSegment=Math.min(11,Math.floor(elapsed/7200000));
      const historicalBySegment=Array.from({length:12},()=>[]);
      const valid=(rows||[]).filter(r=>Number.isFinite(Number(r?.start))&&Number.isFinite(Number(r?.mean))).sort((a,b)=>Number(a.start)-Number(b.start));
      const dayKey=t=>{const d=new Date(t);return d.getFullYear()+"-"+d.getMonth()+"-"+d.getDate();};
      const seenDays=new Set();
      for(let i=0;i<valid.length;i++){
        const t=Number(valid[i].start),d=new Date(t);
        const key=dayKey(t);
        if(d>=today)continue;
        const dayStart=new Date(d);dayStart.setHours(0,0,0,0);
        const seg=Math.max(0,Math.min(11,Math.floor((t-dayStart.getTime())/7200000)));
        const next=i+1<valid.length?Number(valid[i+1].start):t+300000;
        const a=Math.max(t,dayStart.getTime()+seg*7200000);
        const b=Math.min(next,dayStart.getTime()+(seg+1)*7200000);
        if(b>a) {
          const arr=historicalBySegment[seg];
          let item=arr.find(x=>x.day===key);
          if(!item){item={day:key,energy:0};arr.push(item);}
          item.energy+=this._powerValue(Number(valid[i].mean))*(b-a)/3600000;
        }
        seenDays.add(key);
      }
      const segments=[];
      for(let seg=0;seg<12;seg++){
        const segStart=today.getTime()+seg*7200000,segEnd=segStart+7200000;
        const elapsedMs=seg<currentSegment?7200000:seg===currentSegment?Math.max(0,elapsed-seg*7200000):0;
        if(elapsedMs<=0){segments.push({progress:0,color:"future",ratio:null,average:null,samples:0});continue;}
        const actual=this._statisticsEnergy(valid,segStart,Math.min(segEnd,now.getTime()));
        const clean=historicalBySegment[seg].map(x=>x.energy).filter(v=>Number.isFinite(v)&&v>=0).slice(-30);
        const average=clean.length?clean.reduce((a,b)=>a+b,0)/clean.length:null;
        const ratio=average>0?actual/average:null;
        const color=ratio==null?"neutral":ratio>1.10?"red":ratio>=0.90?"yellow":"green";
        segments.push({progress:Math.min(1,elapsedMs/7200000),color,ratio,average,samples:clean.length,actual});
    ...
    async _loadCostHistory(){
      ...
      try{
        const peakCostId=this._resolveEntity(this._cfg.peak_cost_entity,["peak","cost"]);
        const offPeakCostId=this._resolveEntity(this._cfg.off_peak_cost_entity,["off","peak","cost"]);
        const peakEnergyId=this._resolveEntity("sensor.dte_house_energy_peak",["peak","energy"]);
        const peakRateId=this._resolveEntity("input_number.dte_peak_base_rate",["peak","rate"]);
        const ids=[peakCostId,offPeakCostId].filter(Boolean);
        let off=null,peak=null;
        ...
        if(ids.length){
          ...
          const delta=id=>{
            ...
            const first=nums.length?nums[0]:null,last=nums.length?nums[nums.length-1]:current;
            return first==null||last==null?null:Math.max(0,last-first);
          };
          off=delta(offPeakCostId);
          let peakEnergy=null;
          if(peakEnergyId){
            try{
              const ps=await this._ws({type:"recorder/statistics_during_period",start_time:start.toISOString(),end_time:end.toISOString(),statistic_ids:[peakEnergyId],period:"5minute",types:["change","state"]});
              const pr=(ps&&ps[peakEnergyId])||[];
              const changes=pr.map(x=>Number(x.change)).filter(Number.isFinite).reduce((s,v)=>s+Math.max(0,v),0);
              if(pr.length)peakEnergy=changes;
            }catch(e){console.warn("EnergyIQ peak statistics unavailable",e);}
          }
          if(peakEnergy==null&&peakEnergyId)peakEnergy=delta(peakEnergyId);
          const rate=this._state(peakRateId);
          peak=peakEnergy==null||rate==null?null:Math.max(0,peakEnergy*rate);
        }
        const configuredTotal=this._resolveEntity(this._cfg.cost_entity,["energy","cost"]);
        ...
        if(ids.length){
          ...
          const peakSamples=this._historicalMovingDeltas((learnedHistory&&learnedHistory[peakCostId])||[],this._costPeriod,start,end,spec.count);
          const offSamples=this._historicalMovingDeltas((learnedHistory&&learnedHistory[offPeakCostId])||[],this._costPeriod,start,end,spec.count);
          this._costLearned={peak:this._learnedBar(peak,peakSamples),off:this._learnedBar(off,offSamples)};
        }else this._costLearned=null;
    ...
    _brandIconUrl(){const base="/api/brands/integration/energyiq/icon.png";return this._brandToken?base+"?token="+encodeURIComponent(this._brandToken)+"&v=31449":base+"?placeholder=no&v=31449";}
