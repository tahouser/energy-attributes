/* EnergyIQ dashboard card — 3.1.178 */
  ...
  class EnergyIQCard extends HTMLElement {
    ...
    _cost(total,peak,off){
      ...
      const bar=(label,item)=>{
        const value=item.current||0,scale=Math.max(item.max||0,value,0.01),width=Math.max(3,Math.min(100,value/scale*100));
        return '<div class="cost-bar-row"><div class="cost-bar-head"><span>'+label+'</span><strong>'+money(value)+'</strong></div><div class="cost-bar-track"><div class="cost-bar-fill '+item.color+'" style="width:'+width.toFixed(1)+'%"></div></div></div>';
      };
