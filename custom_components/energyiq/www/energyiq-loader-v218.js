/* EnergyIQ v2.1.8 frontend loader. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=218`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub) sub.textContent = "Whole-home electrical intelligence · v2.1.8";
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched218) {
      ...
      proto.__energyiqVersionPatched218 = true;
    }
  }
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver218) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver218 = observer;
  }
