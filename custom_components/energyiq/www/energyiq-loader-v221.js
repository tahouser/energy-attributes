/* EnergyIQ v2.2.1 frontend loader. */
(async () => {
  const load = async (path) => {
    try { await import(`${path}?v=221`); return true; }
  ...
  const setVersion = (panel) => {
    ...
    if (sub) sub.textContent = "Whole-home electrical intelligence · v2.2.1";
  };
  ...
  if (proto) {
    ...
    if (typeof proto._render === "function" && !proto.__energyiqVersionPatched221) {
      ...
      proto.__energyiqVersionPatched221 = true;
    }
  }
  await load("./energyiq-ui-v214.js");
  await load("./energyiq-ui-v216.js");
  await load("./energyiq-ui-v217.js");
  await load("./energyiq-ui-v220.js");
  await load("./energyiq-state-v220.js");
  ...
  applyVersion();
  if (panel && !panel.__energyiqVersionObserver221) {
    const observer = new MutationObserver(applyVersion);
    observer.observe(panel, { childList: true, subtree: true, characterData: true });
    panel.__energyiqVersionObserver221 = observer;
  }
