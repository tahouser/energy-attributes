  ...
  async start() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .label { font-size:18px; letter-spacing:.16em; text-transform:uppercase; opacity:.65; }
        ...
        .stats { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; border-top:1px solid var(--divider-color,#ddd); padding-top:22px; }
    ...
    try {
      const snapshot = await this._hass.callWS({ type: "energyiq_instrument/snapshot" });
      this.render(snapshot);
      await this._hass.connection.subscribeMessage(
        data => this.render(data),
        { type: "energyiq_instrument/subscribe" }
      );
    } catch (err) {
...
customElements.define("energyiq-instrument-panel-v160", EnergyIQInstrumentPanel);
