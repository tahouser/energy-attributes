"""High-rate whole-home measurement coordinator."""
...
class InstrumentCoordinator(DataUpdateCoordinator[dict]):
    """Capture pushed HA measurements without tying acquisition to polling."""

    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        self.hass = hass
        self.entry = entry
        self.power_entity = entry.options.get("power_entity", entry.data["power_entity"])
    ...
    async def async_config_entry_first_refresh(self) -> None:
        ...
        self._unsub_state = async_track_state_change_event(
            self.hass, [self.power_entity], self._state_changed
        )
